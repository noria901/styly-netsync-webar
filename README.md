...
```html
<a-entity image-target="name: sample-target; persist: true">
```

ワールドトラッキング（SLAM）が動いていれば、最後に認識した位置に空間固定されたまま残る。
`disableWorldTracking: true` にしていると空間参照がないのでカメラに追従してしまう。
